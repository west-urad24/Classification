...
class SWEEncoder_ja:
    def __init__(self, bpe, emoji):
        ...
        #今回,𫝆回,今迴,今囬,今𡇌,今囘,今𩶠,𫝆迴,𫝆囬,𫝆𡇌,𫝆囘,𫝆𩶠
        # else b.split(',')
        self.swe = {}
    ...
    def __len__(self):
        return len(self.bpe)#トークンの数を返す

    def clean_text(self, content):
        content = self.content_repatter1.sub("<URL>" ,content)#URL
        content = self.content_repatter2.sub("<EMAIL>" ,content)#メール
        content = self.content_repatter3.sub("<TEL>" ,content)#電話番号
        content = self.content_repatter4.sub("<DATE>" ,content)#日付
        content = self.content_repatter5.sub("<DATE>" ,content)#日付
        content = self.content_repatter6.sub("<PRICE>" ,content)#値段
        content = content.translate(self.content_trans1)
        while '<BLOCK><BLOCK>' in content:
            content = content.replace('<BLOCK><BLOCK>', '<BLOCK>')#ブロック記号
        return content

    def encode(self, text, clean=False):#テキストの前処理
        text = text.replace(' ', '<SP>')#スペース文字
        text = text.replace('　', '<SP>')#スペース文字
        text = text.replace('\r\n', '<BR>')#ブロック記号
        text = text.replace('\n', '<BR>')#ブロック記号
        text = text.replace('\r', '<BR>')#ブロック記号
        text = text.replace('\t', '<TAB>')#タブ
        text = text.replace('—', 'ー')
        text = text.replace('−', 'ー')#
        ...
        def checkkigou(x):#記号かどうか
            e = x.encode()
            if len(x) == 1 and len(e)==2:#1文字かつ、かつ、2バイト
                c = (int(e[0])<<8)+int(e[1])
        ...
        def checku2e(x):#3バイト記号かどうか
            e = x.encode()
            if len(x) == 1 and len(e)==3:#1文字かつ、かつ、3バイト
        ...
        pos = 0
        result = []
        while pos < len(text):#現在地から最後に達するまでループ
            ...
            if len(kouho) > 0:
                ...
                wd = text[pos:end]
                if checkkigou(wd):#記号ならば「KIGOU」
                    result.append(self.swe['<KIGOU>'])
                elif checku2e(wd):#3バイト記号ならば「KIGOU」
...
if __name__=='__main__':#メイン関数
    ...
    with open(args.vocabulary, encoding='utf-8') as f:#ja-swe32k.txt
        bpe = f.read().split('\n')#一行ずつ読み込む
    with open('emoji.json', encoding='utf-8') as f:
        emoji = json.loads(f.read())
    enc = SWEEncoder_ja(bpe, emoji)#SWEEncoder_jaクラスに移動
